...
QFrame* createProductCard(const Products& product) {
    ...
    QObject::connect(buyBtn, &QPushButton::clicked, [product]() {
        // Add to current user's cart (now goes through CART's undo-aware path)
...
QWidget* buildCartPage(QStackedWidget *stackedWidget) {
    ...
    QObject::connect(undoBtn, &QPushButton::clicked, [stackedWidget]() {
        if (currentUser.getShoppingCart().undo()) {
    ...
    QObject::connect(redoBtn, &QPushButton::clicked, [stackedWidget]() {
        if (currentUser.getShoppingCart().redo()) {
            UserStorage::saveUser(currentUser);
            if (stackedWidget) {
                QWidget *oldCart = stackedWidget->widget(1);
                if (oldCart) {
    ...
    QObject::connect(checkoutBtn, &QPushButton::clicked, [totalPrice, stackedWidget]() {
        ...
        currentUser.setBalance(bal - finalTotal);
...
int main(int argc, char *argv[]) {
    ...
    gUserLabel = new QLabel(QString("User: %1")
        .arg(QString::fromStdString(currentUser.getUsername())));
    gUserLabel->setStyleSheet("color: #bbb; font-size: 13px;");
    sidebarLayout->addWidget(gUserLabel);

    gBalanceLabel = new QLabel(QString("Balance: $%1")
